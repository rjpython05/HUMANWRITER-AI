...
/**
 * UAPA Repository Scraper
 * Scrapes institutional repository from rai.uapa.edu.do
 */
export class UAPAScraper extends BaseScraper {
  constructor() {
    const config = getSourceById('uapa');
    if (!config) {
      throw new Error('UAPA source configuration not found');
    }
    super(config);
  }

  async scrape(): Promise<void> {
    logger.info('Starting UAPA scraper', { url: this.sourceConfig.url });

    try {
      // UAPA uses DSpace repository (RAI - Repositorio Académico Institucional)
      ...
      logger.error('UAPA scraper failed', { error: error.message, stack: error.stack });
      throw error;
    }
  }
  ...
  private async discoverCollections(): Promise<string[]> {
    ...
    try {
      ...
      logger.info(`Found ${collections.length} collections in UAPA`, { count: collections.length });
      return collections.slice(0, 20);
    } catch (error: any) {
      logger.error('Failed to discover UAPA collections', { error: error.message });
      return [];
    }
  }
  ...
  private async scrapeCollection(collectionUrl: string): Promise<void> {
    logger.info('Scraping UAPA collection', { url: collectionUrl });
    ...
    try {
      ...
      while (page < maxPages) {
        ...
        if (items.length === 0) {
          logger.info('No more items found in UAPA collection', { page, collectionUrl });
          break;
        }
      ...
      logger.error('Failed to scrape UAPA collection', { collectionUrl, error: error.message });
    }
  }
  ...
  private async scrapeItem(itemUrl: string): Promise<void> {
    try {
      ...
      if (!pdfLink) {
        logger.warn('No PDF found for UAPA item', { itemUrl, title });
      ...
      logger.error('Failed to scrape UAPA item', { itemUrl, error: error.message });
      this.stats.documentsFailed++;
    }
  }
}

export default UAPAScraper;
