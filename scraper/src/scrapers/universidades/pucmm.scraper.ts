...
/**
 * PUCMM Repository Scraper
 * Scrapes academic repository from repositorio.pucmm.edu.do
 */
export class PUCMMScraper extends BaseScraper {
  constructor() {
    const config = getSourceById('pucmm');
    if (!config) {
      throw new Error('PUCMM source configuration not found');
    }
    super(config);
  }

  async scrape(): Promise<void> {
    logger.info('Starting PUCMM scraper', { url: this.sourceConfig.url });

    try {
      // PUCMM uses DSpace repository
      ...
      logger.error('PUCMM scraper failed', { error: error.message, stack: error.stack });
      throw error;
    }
  }
  ...
  private async discoverCollections(): Promise<string[]> {
    ...
    try {
      ...
      logger.info(`Found ${collections.length} collections in PUCMM`, { count: collections.length });
      return collections.slice(0, 20);
    } catch (error: any) {
      logger.error('Failed to discover PUCMM collections', { error: error.message });
      return [];
    }
  }
  ...
  private async scrapeCollection(collectionUrl: string): Promise<void> {
    logger.info('Scraping PUCMM collection', { url: collectionUrl });
    ...
    try {
      ...
      while (page < maxPages) {
        ...
        // Find document items
        const items = $('div.artifact-description, div.item-title, tr.ds-table-row');

        if (items.length === 0) {
          logger.info('No more items found in PUCMM collection', { page, collectionUrl });
          break;
        }
      ...
      logger.error('Failed to scrape PUCMM collection', { collectionUrl, error: error.message });
    }
  }
  ...
  private async scrapeItem(itemUrl: string): Promise<void> {
    try {
      ...
      const yearText = $('meta[name="DC.date"], meta[name="DC.date.issued"]').attr('content') ||
                      $('span.date, div.simple-item-view-date').first().text();
      const year = this.extractYear(yearText || '') || new Date().getFullYear();

      // Find PDF download link
      const pdfLink = $('a[href$=".pdf"], a.btn-primary[href*="bitstream"], div.file-link a[href$=".pdf"]').first().attr('href');

      if (!pdfLink) {
        logger.warn('No PDF found for PUCMM item', { itemUrl, title });
      ...
      logger.error('Failed to scrape PUCMM item', { itemUrl, error: error.message });
      this.stats.documentsFailed++;
    }
  }
}

export default PUCMMScraper;
