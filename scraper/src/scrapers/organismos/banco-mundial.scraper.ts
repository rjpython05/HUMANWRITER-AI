...
/**
 * World Bank Scraper
 * Scrapes development documents from openknowledge.worldbank.org
 * Filters by Dominican Republic
 */
export class BancoMundialScraper extends BaseScraper {
  constructor() {
    const config = getSourceById('banco-mundial');
    if (!config) {
      throw new Error('Banco Mundial source configuration not found');
    }
    super(config);
  }

  async scrape(): Promise<void> {
    logger.info('Starting World Bank scraper', { url: this.sourceConfig.url });

    try {
      // Search for Dominican Republic publications
      await this.scrapeByCountry('Dominican Republic');
    } catch (error: any) {
      logger.error('World Bank scraper failed', { error: error.message, stack: error.stack });
      throw error;
    }
  }
  ...
  private async scrapeByCountry(country: string): Promise<void> {
    logger.info(`Searching World Bank for ${country}`, { country });

    try {
      // World Bank search URL
      ...
      while (page < maxPages) {
        ...
        try {
          ...
          if (items.length === 0) {
            logger.info('No more items found in World Bank search', { page, country });
            break;
          }
          ...
          logger.warn('Failed to fetch World Bank search page', { pageUrl, error: error.message });
          break;
        }
      }
    } catch (error: any) {
      logger.error('Failed to search World Bank by country', { country, error: error.message });
    }
  }
  ...
  private async scrapeItem(itemUrl: string): Promise<void> {
    try {
      ...
      // If no authors found, use World Bank
      if (authors.length === 0) {
        authors.push('World Bank');
      }
      ...
      if (!pdfLink) {
        logger.warn('No PDF found for World Bank item', { itemUrl, title });
      ...
      logger.error('Failed to scrape World Bank item', { itemUrl, error: error.message });
      this.stats.documentsFailed++;
    }
  }
}

export default BancoMundialScraper;
